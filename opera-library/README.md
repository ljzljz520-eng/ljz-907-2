...
- CSV 导入：批量导入剧目（含封面与剧照关联），自动建剧种，同名同剧种执行更新，事务导入、逐行错误报告，兼容 UTF-8 / GBK 编码
...
├── includes/              # 引导、PDO 连接、公共函数、CSV 导入逻辑、前台布局
...
首行表头可省略，列顺序：`剧目名称*, 剧种*, 地区, 传承人, 演员, 年代, 视频链接, 简介, 封面, 剧照`

- 剧种不存在时自动创建
- 「剧目名称 + 剧种」相同视为同一剧目，重复导入执行更新
- 封面 / 剧照：填 `uploads/` 目录下的相对路径（如 `seed/mudanting_1.svg`），剧照多张用 `|` 分隔；图片需提前放入 `uploads/`，路径不存在时忽略该图片并在结果中提示
- 更新已有剧目时，仅当封面 / 剧照列非空才覆盖对应图片
- 整个文件在一个事务中导入，出错自动回滚

> 示例 CSV（`sample_data/plays_sample.csv`）中的视频为可播放的演示 mp4 直链，正式使用请替换为真实演出影像链接（支持 mp4 直链 / B站 / YouTube）。
